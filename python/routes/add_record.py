...
def proveedores():
    ...
    if form_proveedores.validate_on_submit():
        id = (
            db.session.query(PortalServiciosProveedores)
            .order_by(PortalServiciosProveedores.id_proveedor.desc())
            .first()
        )
        if id is None:
            id = "PR1"
...
def add_contratos():
    ...
    proveedores_select = [("", "Selecciona una opción")] + [
        (str(proveedor.id_proveedor), proveedor.nombre)
        for proveedor in table_proveedores
    ]
    form_contratos.id_proveedor.choices = proveedores_select
    if form_contratos.validate_on_submit():
        id = (
            db.session.query(PortalServiciosContratos)
            .order_by(PortalServiciosContratos.id_contrato.desc())
            .first()
        )
        if id is None:
            id = "CO1"
...
def servicios_medicos():
    ...
    id = (
        db.session.query(PortalServiciosServiciosMedicos)
        .order_by(PortalServiciosServiciosMedicos.id_servicio_medico.desc())
        .first()
    )
    if form_servicios_medicos.validate_on_submit():
        ...
        servicio_medico = PortalServiciosServiciosMedicos(
            id_servicio_medico=id, nombre=form_servicios_medicos.nombre.data
        )
...
def ubicaciones():
    ...
    if form_ubicaciones.validate_on_submit():
        id = (
            db.session.query(PortalServiciosUbicaciones)
            .order_by(PortalServiciosUbicaciones.id_ubicacion.desc())
            .first()
        )
        if id is None:
            id = "UB1"
        ...
        ubicacion = PortalServiciosUbicaciones(
            id_ubicacion=id, nombre=form_ubicaciones.nombre.data
        )
...
def usuarios():
    ...
    roles_select = [("", "Selecciona una opción")] + [
        (str(rol.id_rol), rol.nombre) for rol in table_roles
    ]
    ...
    if form_usuarios.validate_on_submit():
        id = (
            db.session.query(PortalServiciosUsuarios)
            .order_by(PortalServiciosUsuarios.id_usuario.desc())
            .first()
        )
