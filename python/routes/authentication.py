...
def login_submit():
    ...
    user = PortalServiciosUsuarios.query.filter(
        PortalServiciosUsuarios.correo_electronico == email
    ).first()
    if user != None:
        if check_password_hash(user.contrasena, contrasena):
            ...
            rol = PortalServiciosRoles.query.filter(
                PortalServiciosRoles.id_rol == user.id_rol
            ).first()
