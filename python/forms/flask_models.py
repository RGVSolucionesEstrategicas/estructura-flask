...
class UsuariosForm(FlaskForm):
    ...
    id_proveedor = SelectField(
        "Proveedor*", validators=[DataRequired(), Length(max=50)]
    )
    nombre = StringField("Nombre*", validators=[DataRequired(), Length(max=50)])
    correo_electronico = EmailField(
        "Correo electrónico*", validators=[DataRequired(), Length(max=50)]
    )
...
class ProveedoresForm(FlaskForm):
    ...
    razon_social = StringField(
        "Razón Social*", validators=[DataRequired(), Length(max=50)]
    )
    estatus = SelectField(
        "Estatus",
        choices=[("Activo", "Activo"), ("Inactivo", "Inactivo")],
...
class ClavesOracleForm(FlaskForm):
    ...
    clave_cuadro_basico = StringField(
        "Clave cuadro básico", validators=[Length(max=50)]
    )
    descripcion = TextAreaField(
        "Descripción*", validators=[DataRequired(), Length(max=1000)]
    )
...
class ContratosForm(FlaskForm):
    ...
    numero_contrato = StringField(
        "Número de contrato*", validators=[DataRequired(), Length(max=50)]
    )
    estatus = SelectField(
        "Estatus",
        choices=[("Activo", "Activo"), ("Inactivo", "Inactivo")],
...
class ClavesOracleEnContratoForm(FlaskForm):
    id_clave_oracle_en_contrato = StringField(
        "ID Clave oracle contrato", validators=[Length(max=50)]
    )
    id_contrato = StringField("Contrato*", validators=[DataRequired(), Length(max=255)])
    clave_oracle = StringField(
        "Clave oracle*", validators=[DataRequired(), Length(max=50)]
    )
...
class ClavesOracleEntregadasForm(FlaskForm):
    id_clave_oracle_entregada = StringField(
        "ID clave oracle entregada", validators=[Length(max=50)]
    )
    id_clave_oracle_en_contrato = StringField(
        "ID Clave oracle contrato", validators=[DataRequired(), Length(max=255)]
    )
    id_ubicacion = StringField(
        "Ubicación*", validators=[DataRequired(), Length(max=255)]
    )
    id_servicio_medico = StringField(
        "Servicio médico*", validators=[DataRequired(), Length(max=255)]
    )
    clave_oracle = StringField(
        "Clave oracle*", validators=[DataRequired(), Length(max=50)]
    )
    cantidad_entregada = DecimalField(
        "Cantidad entregada*", validators=[DataRequired()]
    )
